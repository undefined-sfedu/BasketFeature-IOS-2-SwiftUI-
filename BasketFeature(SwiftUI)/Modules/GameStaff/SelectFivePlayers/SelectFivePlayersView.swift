...
struct SelectFivePlayersView: View {
        ...
        HStack{
            ForEach(viewModel.selectedPLayers, id: \.self) { item in
                Button {
                    ...
                    RoundedRectangle(cornerRadius: 10)
                        .strokeBorder(Color.appOrange, lineWidth: 2)
                        .overlay(
                            Text("\(item)")
    ...
    var NextScreenButton: some View{
        Button {
            ...
            Text("Далее")
                .font(.system(size: 24))
                .foregroundColor( viewModel.selectedPLayers.count >= 5 ? .appOrange : .appGray)
                .padding(.vertical)
                .frame(maxWidth: .infinity)
                .background(
                    RoundedRectangle(cornerRadius: 10)
                        .strokeBorder(viewModel.selectedPLayers.count >= 5 ? Color.appOrange : Color.appGray, lineWidth: 2.5)
                        .background(viewModel.selectedPLayers.count >= 5 ? Color.white.cornerRadius(10) : Color.appGray.opacity(0.3).cornerRadius(10))
                )
        }
